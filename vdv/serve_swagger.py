...
class SpecServer():
    ...
    def load_spec_swagger(self, swagger_spec):
        ...
        try:
            swagger_spec = json.loads(swagger_spec, object_pairs_hook=OrderedDict)
        except:
    ...
    def handle_preflight_request(self, req, resp):
        ...
        if req.method == 'OPTIONS':
            ...
            resp.set_header('Access-Control-Allow-Headers', 'X-Auth-Token, Keep-Alive, Users-Agent, X-Requested-With, If-Modified-Since, Cache-Control, Content-Type')
            ...
            for operation in self.routing_table.items():
                response_body += repr(operation) + '\n'
            response_body += '\n'
            for operation in self.routing_table:
    ...
    def match_request_url_to_operation(self, req, resp):
        ...
        uri_fields = None
        for operation, v in self.routing_table.items():
            ...
            m = v['uri_template'].match(route_signature.partition('?')[0])
            if m:
                ...
                uri_fields = m.groupdict()
                log.info("Request URL contained these template fields: {}".format(uri_fields))
                matched_operation = operation
                log.info("Matched operation: {} {}".format(matched_operation, v))
                break
        ...
        if matched_operation is None:
            ...
            matched_operation = "httpDefault"

        # the request URL matches an operation from our Swagger specification, but does not have request_handlers so return 404
        if matched_operation not in self.operation_handlers.keys():
            log.info("Operation found in spec but not in operation_handlers: {}".format(matched_operation))
            raise falcon.HTTPNotFound()

        return matched_operation, uri_fields
    ...
    def dispatch_matched_operation_to_request_handlers(self, req, resp, matched_operation, uri_fields):
        ...
        matched_operation_handlers = self.operation_handlers[matched_operation]
        request_handler_args = {
            'req': req,
            'resp': resp,
            'config': self.config,
            'uri_fields': uri_fields,
            'operation_id': matched_operation,
        }
